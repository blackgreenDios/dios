                ...
                const appendComment = (commentObject, isSub) => {
                    const commentHtmlText = `
                     ...
                     ${commentObject['isLiked'] ? 'liked' : ''}" rel="comment">
                            ...
                            <div class="body">
                                <div class="content">
                                    <span class="text ${commentObject['isSecret']? 'secret': '' }" >${commentObject['content']}</span>
                                    <div class="like">
                                ...
                                <form class="modify-form" rel="modifyForm">
                                    ...
                                    <div class="secret" id="modifySecret">
                                    <a  class="toggle">
                                <i rel="secret" class="secretIcon fa-solid fa-lock"></i>
                                <i rel="open" class="openIcon visible fa-solid fa-lock-open"></i>                                    </a>
                                </div>
                     ...
                     <form class="reply-form" rel="replyForm">
                        ...
                        <div class="body">
                            ...
                            <div class="secret" id="replySecret">
                                    <a  class="toggle">
                                <i rel="secret" class="secretIcon fa-solid fa-lock"></i>
                                <i rel="open" class="openIcon visible fa-solid fa-lock-open"></i>                                    </a>
                                </div>
                    ...
                    // 수정 비밀댓글
                    modifyFormElement.querySelector('[rel="open"]').addEventListener('click', () => {
                        modifyFormElement.querySelector('[rel="open"]').classList.remove('visible');
                        modifyFormElement.querySelector('[rel="secret"]').classList.add('visible');
                        isSecret = true;
                    });

                    modifyFormElement.querySelector('[rel="secret"]').addEventListener('click', () => {
                        modifyFormElement.querySelector('[rel="secret"]').classList.remove('visible');
                        modifyFormElement.querySelector('[rel="open"]').classList.add('visible');
                        isSecret = false;
                    });
                    ...
                    //대댓글 비밀댓글
                    replyFormElement.querySelector('[rel="open"]').addEventListener('click', () => {
                        replyFormElement.querySelector('[rel="open"]').classList.remove('visible');
                        replyFormElement.querySelector('[rel="secret"]').classList.add('visible');
                        isSecret = true;
                    });

                    replyFormElement.querySelector('[rel="secret"]').addEventListener('click', () => {
                        replyFormElement.querySelector('[rel="secret"]').classList.remove('visible');
                        replyFormElement.querySelector('[rel="open"]').classList.add('visible');
                        isSecret = false;
                    });
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                if (responseObject['result'] === 'success') {
                    if (responseObject['isLiked'] === true) {
                        document.querySelector('[rel="toggle"]').classList.add('liked');
                    } else {
                        document.querySelector('[rel="toggle"]').classList.remove('liked');
                    }
                    document.querySelector('[rel="liked"]').innerText = responseObject['likeCount'];
                }

                else if (responseObject['result'] === 'not') {
